  ...
  try {
    ...
    if (driver) {
      filteredDeliveries = filteredDeliveries.filter(d =>
        d.driver.toLowerCase().includes((driver as string).toLowerCase())
      );
    }

    if (route) {
      filteredDeliveries = filteredDeliveries.filter(d =>
        d.route.toLowerCase().includes((route as string).toLowerCase())
      );
    }
...
// @access  Private
export const optimizeRoutes = async (req: AuthRequest, res: Response) => {
  try {
