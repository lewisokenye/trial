...
// @access  Private
export const getLocalFoods = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (season && season !== 'all') {
      filteredFoods = filteredFoods.filter(food =>
        food.season.toLowerCase().includes((season as string).toLowerCase()) ||
...
// @access  Private
export const getNutritionRecommendations = async (req: AuthRequest, res: Response) => {
  try {
...
// @access  Private
export const calculateNutritionNeeds = async (req: AuthRequest, res: Response) => {
  try {
...
export const generateMealPlanValidation = [
  ...
  body('activityLevel').optional().isIn(['sedentary', 'light', 'moderate', 'active', 'very-active']).withMessage('Invalid activity level'),
  body('budget').optional().isNumeric().withMessage('Budget must be a number')
];

export const calculateNeedsValidation = [
  body('age').isNumeric().withMessage('Age is required and must be a number'),
  ...
  body('activityLevel').isIn(['sedentary', 'light', 'moderate', 'active', 'very-active']).withMessage('Invalid activity level'),
